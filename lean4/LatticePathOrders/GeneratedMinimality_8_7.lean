import LatticePathOrders.GeneratedMinimality_8_7_Frontier
import LatticePathOrders.GeneratedMinimality_8_7_Group000
...
theorem regions_valid : ∀ region ∈ regions,
    region.1.length + region.2.1 = 15 ∧
      region.2.2.check 8 7 classes region.2.1 region.1 = true := by
  intro region h
  exact regionsGroup000_valid region h

theorem regions_cover : TreeRegionsCover 8 7 classes regions partitionTree :=
  ⟨classes_valid, partitionTree_check, regions_frontier, regions_valid⟩

theorem problem63_8_7 : Problem63HoldsAt 8 7 :=
