import LatticePathOrders.GeneratedMinimality_2_1_Data
...
def region_0000 : AssignmentTree :=
  (.node (some (.node none (some (.leaf 0 false 0)))) none)

theorem region_0000_check :
    AssignmentTree.check 2 1 classes 2 (mustParsePath "R") region_0000 = true := by
  unfold region_0000
  rfl

def regionsPart000 : List (Path × Nat × AssignmentTree) := [
  (mustParsePath "R", 2, region_0000)
]
