import LatticePathOrders.GeneratedMinimality_9_4_Frontier
import LatticePathOrders.GeneratedMinimality_9_4_Group000
...
theorem regions_valid : ∀ region ∈ regions,
    region.1.length + region.2.1 = 13 ∧
      region.2.2.check 9 4 classes region.2.1 region.1 = true := by
  intro region h
  exact regionsGroup000_valid region h

theorem regions_cover : TreeRegionsCover 9 4 classes regions partitionTree :=
  ⟨classes_valid, partitionTree_check, regions_frontier, regions_valid⟩

theorem problem63_9_4 : Problem63HoldsAt 9 4 :=
