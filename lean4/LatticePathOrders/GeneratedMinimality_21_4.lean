import LatticePathOrders.GeneratedMinimality_21_4_Frontier
import LatticePathOrders.GeneratedMinimality_21_4_Group000
...
theorem regions_valid : ∀ region ∈ regions,
    region.1.length + region.2.1 = 25 ∧
      region.2.2.check 21 4 classes region.2.1 region.1 = true := by
  intro region h
  exact regionsGroup000_valid region h

theorem regions_cover : TreeRegionsCover 21 4 classes regions partitionTree :=
  ⟨classes_valid, partitionTree_check, regions_frontier, regions_valid⟩

theorem problem63_21_4 : Problem63HoldsAt 21 4 :=
  problem63HoldsAt_of_treeRegionsCover (by decide) (by decide) regions_cover

/-- Endpoint certificate: 11 regions. -/
theorem region_count : regions.length = 11 := by
