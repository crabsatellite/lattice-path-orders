import LatticePathOrders.GeneratedMinimality_5_1_Data
...
def region_0000 : AssignmentTree :=
  (.node (some (.node (some (.node (some (.node (some (.node none (some (.leaf 0 false 0)))) none)) none)) none)) none)

theorem region_0000_check :
    AssignmentTree.check 5 1 classes 5 (mustParsePath "R") region_0000 = true := by
  unfold region_0000
  rfl

def regionsPart000 : List (Path × Nat × AssignmentTree) := [
  (mustParsePath "R", 5, region_0000)
]

theorem regionsPart000_valid : ∀ region ∈ regionsPart000,
    region.1.length + region.2.1 = 6 ∧
      region.2.2.check 5 1 classes region.2.1 region.1 = true := by
  intro region h
