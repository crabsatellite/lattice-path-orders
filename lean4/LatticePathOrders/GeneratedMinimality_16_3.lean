import LatticePathOrders.GeneratedMinimality_16_3_Frontier
import LatticePathOrders.GeneratedMinimality_16_3_Group000
...
theorem regions_valid : ∀ region ∈ regions,
    region.1.length + region.2.1 = 19 ∧
      region.2.2.check 16 3 classes region.2.1 region.1 = true := by
  intro region h
  exact regionsGroup000_valid region h

theorem regions_cover : TreeRegionsCover 16 3 classes regions partitionTree :=
  ⟨classes_valid, partitionTree_check, regions_frontier, regions_valid⟩

theorem problem63_16_3 : Problem63HoldsAt 16 3 :=
