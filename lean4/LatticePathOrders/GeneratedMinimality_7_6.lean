import LatticePathOrders.GeneratedMinimality_7_6_Frontier
import LatticePathOrders.GeneratedMinimality_7_6_Group000
...
theorem regions_valid : ∀ region ∈ regions,
    region.1.length + region.2.1 = 13 ∧
      region.2.2.check 7 6 classes region.2.1 region.1 = true := by
  intro region h
  exact regionsGroup000_valid region h

theorem regions_cover : TreeRegionsCover 7 6 classes regions partitionTree :=
  ⟨classes_valid, partitionTree_check, regions_frontier, regions_valid⟩

theorem problem63_7_6 : Problem63HoldsAt 7 6 :=
