import LatticePathOrders.GeneratedMinimality_8_1_Data
...
def region_0000 : AssignmentTree :=
  (.node (some (.node (some (.node (some (.node (some (.node (some (.node (some (.node (some (.node none (some (.leaf 0 false 0)))) none)) none)) none)) none)) none)) none)) none)

theorem region_0000_check :
    AssignmentTree.check 8 1 classes 8 (mustParsePath "R") region_0000 = true := by
  unfold region_0000
  rfl
